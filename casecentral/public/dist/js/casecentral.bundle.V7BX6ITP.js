(() => {
  ...
  // ../casecentral/casecentral/public/js/sales_invoice.js
  ...
  var get_legal_services_to_invoice = function(frm) {
    ...
    dialog.fields_dict["matter"].df.onchange = () => {
      ...
      if (matter && matter != selected_matter) {
        ...
        var args = { matter, company: frm.doc.company };
    ...
    $results.on("click", ".list-item--head :checkbox", (e) => {
      $results.find(".list-item-container .list-row-check").prop("checked", $(e.target).is(":checked"));
    });
  ...
  var make_list_row = function(columns, result = {}) {
    ...
    columns.forEach(function(column) {
      contents += `<div class="list-item__content ellipsis">
			${head ? `<span class="ellipsis">${__(frappe.model.unscrub(column))}</span>` : column !== "name" ? `<span class="ellipsis">${__(result[column])}</span>` : `<a class="list-id ellipsis">
						${__(result[column])}</a>`}
		</div>`;
    });
  ...
  var list_row_data_items = function(head, $row, result) {
    head ? $row.addClass("list-item--head") : $row = $(`<div class="list-item-container"
...
//# sourceMappingURL=casecentral.bundle.V7BX6ITP.js.map
