...
def execute(filters=None):
	...
	matter = frappe.qb.DocType("Matter")
	query = frappe.qb.from_(matter).select(
		matter.name.as_("matter"),
		matter.customer,
		matter.customer_name,
		matter.posting_date,
		matter.status,
		matter.matter_type,
		matter.service_type,
	...
	if filters.get("matter_type"):
		query = query.where(matter.matter_type == filters.get("matter_type"))
	if filters.get("service_type"):
		query = query.where(matter.service_type == filters.get("service_type"))
	if filters.get("service"):
		query = query.where(matter.service == filters.get("service"))
	if filters.get("customer"):
		query = query.where(matter.customer == filters.get("customer"))
	if filters.get("from_date"):
		query = query.where(matter.posting_date >= getdate(filters["from_date"]))
	if filters.get("to_date"):
		query = query.where(matter.posting_date <= getdate(filters["to_date"]))

	query = query.orderby(matter.posting_date)
	matters = query.run(as_dict=True)
