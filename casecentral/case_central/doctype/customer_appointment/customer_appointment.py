...
class CustomerAppointment(Document):
	...
	def validate_overlaps(self):
		...
		ca = frappe.qb.DocType("Customer Appointment")
		from frappe.query_builder.functions import Function
		Addtime = type('Addtime', (Function,), {'__init__': lambda self, *args: super(Function, self).__init__('ADDTIME', *args)})
		SecToTime = type('SecToTime', (Function,), {'__init__': lambda self, *args: super(Function, self).__init__('SEC_TO_TIME', *args)})

		end_time_expr = Addtime(ca.appointment_time, SecToTime(ca.duration * 60))

		cond_1 = (ca.appointment_time < self.appointment_time) & (end_time_expr > self.appointment_time)
		cond_2 = (ca.appointment_time > self.appointment_time) & (ca.appointment_time < end_time.time())
		cond_3 = (ca.appointment_time == self.appointment_time)

		overlapping_appointments = (
			frappe.qb.from_(ca)
			.select(ca.name, ca.employee, ca.customer, ca.appointment_time, ca.duration, ca.meeting_room)
			.where(
				(ca.appointment_date == self.appointment_date) &
				(ca.name != self.name) &
				(ca.status.notin(["Closed", "Cancelled"])) &
				((ca.employee == self.employee) | (ca.customer == self.customer)) &
...
def get_events(start, end, filters=None):
	...
	appointment_types = frappe.get_all("Appointment Type", fields=["name", "color"])
	colors = {t.name: t.color for t in appointment_types}

	for item in data:
		item.title = f"{item.customer}\r{item.customer_name or ''}"
		item.start = datetime.datetime.combine(
			getdate(item.appointment_date), get_time(item.appointment_time)
		)
		item.end = item.start + datetime.timedelta(minutes=item.duration)
		item.color = colors.get(item.appointment_type) or ""
		item.allDay = 0
